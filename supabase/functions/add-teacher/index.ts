...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!teacher) return jsonResponse({ error: "Apenas o professor pode cadastrar outros professores." }, 403);
    ...
    if (insertError) return jsonResponse({ error: "Erro ao criar professor." }, 500);
