...
Deno.serve(async (req) => {
  ...
  try {
    const { token, teacherId, name, email, password } = await req.json();
    if (!teacherId || !name || !email) return jsonResponse({ error: "Dados incompletos." }, 400);

    const admin = adminClient();
    const teacher = await requireRole(admin, token, "teacher");
    if (!teacher) return jsonResponse({ error: "Apenas o professor pode editar professores." }, 403);
    ...
    const { error: updateError } = await admin.from("users").update(update).eq("id", teacherId).eq("role", "teacher");
    if (updateError) return jsonResponse({ error: "Erro ao atualizar professor." }, 500);
